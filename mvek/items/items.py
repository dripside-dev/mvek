...
# ===========================================================================
#
# Каждый предмет относится к одному из 4 тиров по силе эффекта:
#   1 — обычные (мелкий прирост одной характеристики)
#   2 — крепкие  (двойной стат / полезная утилита)
#   3 — сильные  (механики, меняющие забег: pierce, freeze, familiar…)
#   4 — легендарные (game-changer: revive, +99 монет, 7 орбиталок…)
#
# Чем выше тир, тем меньше вес в случайном выборе и тем дороже в магазине.

TIER_WEIGHTS = {1: 100, 2: 50, 3: 20, 4: 6}
TIER_SHOP_PRICE = {1: 7, 2: 12, 3: 20, 4: 35}
_DEFAULT_TIER = 1
...
ITEM_TIERS = {
    ...
    "Стипендия": 4,
    "Зачётка-автомат": 4,
    "Модель Солнечной системы": 4,
    "Алебастровая шкатулка": 4,

    # ---------- Tier 3: сильные ----------
    "Красный диплом": 3,
    "Очки ботаника": 3,
    "Проездной на трамвай": 3,
    "Стакан компота": 3,
    "Линейка-указка": 3,
    "Указка преподавателя": 3,
    "Молочный коктейль": 3,
    "Помощник-первокурсник": 3,
    "Книга добродетелей": 3,
    "Звезда отличника": 3,
    "Студенческий Wi-Fi": 3,
    "Кнопка переэкзаменовки": 3,
    "Печать декана": 3,
    "Магнит с холодильника": 3,
    "Студийная камера": 3,
    "Театральная маска": 3,
    "Святой щит \"Деканат\"": 3,
    "Откровение деканата": 3,

    # ---------- Tier 2: крепкие ----------
    "Забытая шпаргалка": 2,
    "Зачётка с печатью": 2,
    "Тяжёлый рюкзак": 2,
    "Валентинка от анонима": 2,
    "Студенческий проездной \"Меркурий\"": 2,
    "Свеча на парте": 2,
    "Орбитальный конспект": 2,
    "Пропуск в столовую": 2,
    "Энергетик \"3 часа ночи\"": 2,
    "Чернильница": 2,
    "Идеальная тетрадь": 2,
    "Медаль за учёбу": 2,
    "Гантеля": 2,
    ...
    "Палитра": 2,
    "Кисть": 2,
    "Брусок мыла из туалета": 2,
    "Настольная лампа": 2,

    # ---------- Tier 1: обычные (всё остальное) ----------
    # Кружка кофе, Спортивная повязка, Беговые кроссовки, Перьевая ручка,
    # Шапочка выпускника, Читательский билет, Энциклопедия, Закладка, Лупа,
    # USB-флешка, Нота с прослушки — получают _DEFAULT_TIER.
}

for _it in ITEM_REGISTRY:
    _it["tier"] = ITEM_TIERS.get(_it["name"], _DEFAULT_TIER)


def _weighted_pick(rng: random.Random, pool: list[dict]) -> dict:
    weights = [TIER_WEIGHTS.get(it.get("tier", _DEFAULT_TIER), 1) for it in pool]
    return rng.choices(pool, weights=weights, k=1)[0]


def shop_price(item: dict) -> int:
    return TIER_SHOP_PRICE.get(item.get("tier", _DEFAULT_TIER), 7)


def random_pickup(rng: random.Random) -> dict:
    return _weighted_pick(rng, ITEM_REGISTRY)
...
class ItemPickup(Entity):
    ...
    def draw(self, surface: pygame.Surface, ox: int, oy: int) -> None:
        ...
        if assets.blit_item_icon(surface, self.item, cx, gy, size=28):
            pass
        elif icon:
