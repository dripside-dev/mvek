...
def draw_hud(surface, student, floor, current_room) -> None:
    ...
    if student.active_item:
        ...
        icon = it.get("icon")
        if assets.blit_item_icon(surface, it,
   ...
   # Passives inventory — moved further right so it does not overlap
    ...
    if student.passives:
        ...
        for idx, ite in enumerate(student.passives):
            ...
            drawn = False
            if item is not None:
                drawn = assets.blit_item_icon(surface, item,
                                              center_x, center_y,
                                              size=inv_icon)
                if not drawn and item.get("icon"):
                    paint_icon(surface, item["icon"], center_x, center_y,
                               scale=inv_icon / 28.0)
                    drawn = True
            if not drawn:
                h = hash(str(ite))
                ...
                pygame.draw.circle(surface, (30, 25, 25),
                                   (center_x, center_y), inv_icon // 2, 1)
